---
name: giodb-architecture
description: Use when you need to understand the GioDB project structure, module layout, or system architecture. Provides the module map, dependency graph, and key abstractions.
...
|--------|---------|--------|---------|
| `common` | `include/giodb/common/` | `src/common/` | Result<T>, StatusCode, Error, Value, types, logging, coercion |
| `storage` | `include/giodb/storage/` | `src/storage/` | DiskManager, BufferPoolManager, page I/O, LRU-K eviction |
| `catalog` | `include/giodb/catalog/` | `src/catalog/` | Catalog, TableSchema, column metadata, table ID management |
| `table` | `include/giodb/table/` | `src/table/` | TableHeap, TableIterator, TupleSerializer, slotted pages |
| `parser` | `include/giodb/parser/` | `src/parser/` | Lexer, Token, Parser, AST nodes (expressions + statements) |
| `planner` | `include/giodb/planner/` | `src/planner/` | Binder (semantic analysis), TypeResolver |
| `executor` | `include/giodb/executor/` | `src/executor/` | Volcano iterators, expression evaluator, planner, QueryEngine |
| `index` | `include/giodb/index/` | `src/index/` | B+ tree index (Phase 3) |
| `graph` | `include/giodb/graph/` | `src/graph/` | Graph engine (Phase 4+) |
| `vector` | `include/giodb/vector/` | `src/vector/` | HNSW vector index (Phase 4+) |
| `txn` | `include/giodb/txn/` | `src/txn/` | Transactions, MVCC (Phase 3+) |
| `server` | `include/giodb/server/` | `src/server/` | PostgreSQL wire protocol (Phase 4+) |
