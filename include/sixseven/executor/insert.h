...
public:
    ...
    /// Set the lock manager and table id for locking during INSERT (GDB-930).
    /// When set, acquires IX intent lock on the table before placing rows.
...
private:
    ...
    /// Lock manager for acquiring table IX lock on INSERT (GDB-930).
