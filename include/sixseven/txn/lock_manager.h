...
template <>
struct std::hash<sixseven::TableLockKey> {
    std::size_t operator()(const sixseven::TableLockKey& k) const noexcept {
        return std::hash<sixseven::table_id_t>{}(k.table_id);
    }
};

template <>
struct std::hash<sixseven::RowLockKey> {
    std::size_t operator()(const sixseven::RowLockKey& k) const noexcept {
        auto h1 = std::hash<sixseven::table_id_t>{}(k.table_id);
        auto h2 = std::hash<sixseven::PageId>{}(k.page_id);
        auto h3 = std::hash<sixseven::SlotId>{}(k.slot_id);
        // Combine hashes.
...
template <>
struct std::hash<sixseven::LockKey> {
    std::size_t operator()(const sixseven::LockKey& k) const noexcept {
        return std::visit(
