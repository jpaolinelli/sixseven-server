...
Result<void> EdgeTraversalOperator::collect_edges() {
    ...
    // Number of edge property columns = schema columns - 3 meta-columns (__from, __to, __depth).
    const size_t expected_props = schema_.column_count() > 3 ? schema_.column_count() - 3 : 0;

    // Helper to emit a single edge as a result tuple.
    auto emit_edge = [&](const EdgeRow& edge, int32_t from_depth, int32_t to_depth) {
        Tuple tuple;
    ...
    if (!heterogeneous_) {
        ...
        for (const auto& [node_pk, node_depth] : depths) {
            ...
            for (const auto& edge : *edges) {
                ...
                if (it != depths.end()) {
                    emit_edge(edge, node_depth, it->second);
                }
            }
        }
    } else {
        ...
        if (config_.direction == TraverseDirection::OUT) {
            ...
            for (const auto& [src_pk, src_depth] : source_depths) {
                ...
                for (const auto& edge : *edges) {
                    auto it = target_depths.find(edge.target_pk);
                    if (it != target_depths.end()) {
                        emit_edge(edge, src_depth, it->second);
                    }
                }
            }
        }
