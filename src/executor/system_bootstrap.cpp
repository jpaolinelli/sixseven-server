...
Result<void> SystemBootstrap::seed_default_settings(QueryEngine& engine, const Config& config) {
    ...
    std::vector<Setting> defaults = {
        ...
        {"replication.archive_cleanup_policy",
         ...
         false},
        {"replication.archive_keep_last_n",
         std::to_string(config.archive_keep_last_n),
         "replication",
         "Number of WAL segments to retain when cleanup policy is keep_last_n",
         false},
        ...
        {"replication.synchronous_fallback",
         ...
         true},
        {"replication.lag_warning_threshold_bytes",
         std::to_string(config.replication_lag_warning_threshold_bytes),
         "replication",
         "Replication lag threshold in bytes (current_lsn - applied_lsn) before warning is logged",
         true},
