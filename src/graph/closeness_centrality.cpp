...
Result<std::vector<AlgorithmRow>> closeness_centrality_execute(const AlgorithmContext& ctx) {
    ...
    if (auto it = ctx.named_args.find("variant"); it != ctx.named_args.end()) {
        auto v = value_to_string(it->second, "variant parameter");
        if (!v.has_value()) {
    ...
    for (const auto& edge : *edges) {
        auto src = value_to_int64(edge.source_pk, "closeness centrality");
        if (!src.has_value()) {
            return tl::unexpected(src.error());
        }
        auto tgt = value_to_int64(edge.target_pk, "closeness centrality");
        if (!tgt.has_value()) {
