...
Result<void> HnswIndex::create(const HnswIndexConfig& config) {
    ...
    if (!insert_result.has_value()) {
        ...
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to write HNSW metadata to page");
    }

    auto unpin_result = buffer_pool_.unpin_page(meta_page_id_, true);
    if (!unpin_result.has_value()) {
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to unpin HNSW metadata page");
    }

    GIODB_LOG_INFO("Created HNSW index: dim={}, M={}, ef_c={}, ef_s={}, meta_page={}",
                   config.dimension, config.m, config.ef_construction,
                   config.ef_search, meta_page_id_);
...
Result<void> HnswIndex::load(PageId meta_page_id) {
    ...
    if (!page_result.has_value()) {
        return make_error(StatusCode::IO_ERROR,
                          "Failed to fetch HNSW metadata page");
    }
    ...
    if (!tuple_result.has_value()) {
        (void)buffer_pool_.unpin_page(meta_page_id_, false);
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to read HNSW metadata tuple");
    }
    ...
    GIODB_LOG_INFO("Loaded HNSW index: dim={}, nodes={}, meta_page={}",
                   meta_.dimension, meta_.node_count, meta_page_id_);
...
Result<uint32_t> HnswIndex::insert(std::span<const float> vector) {
    ...
    if (!vec_loc_result.has_value()) {
        return make_error(vec_loc_result.error().code,
                          vec_loc_result.error().message);
    }
    ...
    if (meta_.entry_point_id == hnsw_invalid_node_id) {
        ...
        if (!node_loc_result.has_value()) {
            return make_error(node_loc_result.error().code,
                              node_loc_result.error().message);
        }
    ...
    for (int layer = static_cast<int>(meta_.max_layer);
         layer > static_cast<int>(new_layer); --layer) {
        auto greedy_result = greedy_search_layer(
            vector, current_entry, static_cast<uint8_t>(layer));
    ...
    for (int layer = static_cast<int>(start_layer); layer >= 0; --layer) {
        auto candidates_result = search_layer(
            vector, current_entry, meta_.ef_construction,
            static_cast<uint8_t>(layer), nullptr);
        ...
        uint16_t max_neighbors = (layer == 0)
                                     ? static_cast<uint16_t>(meta_.m_param * 2)
                                     : meta_.m_param;
...
Result<std::vector<HnswSearchResult>>
HnswIndex::search(std::span<const float> query, uint32_t k) const {
    return search(query, k, nullptr);
}

Result<std::vector<HnswSearchResult>>
HnswIndex::search(std::span<const float> query,
                   uint32_t k,
    ...
    for (int layer = static_cast<int>(meta_.max_layer); layer > 0; --layer) {
        auto greedy_result = greedy_search_layer(
            query, current_entry, static_cast<uint8_t>(layer));
        if (greedy_result.has_value()) {
            current_entry = greedy_result.value();
        }
    }
    ...
    if (!candidates_result.has_value()) {
        return make_error(candidates_result.error().code,
    ...
    for (size_t i = 0; i < candidates.size() && i < k; ++i) {
        ...
        if (predicate && !predicate(candidates[i].node_id)) {
            continue;
        }
...
Result<void> HnswIndex::remove(uint32_t node_id) {
    ...
    if (!update_result.has_value()) {
        return make_error(update_result.error().code,
    ...
    for (size_t layer = 0; layer < node.neighbors.size(); ++layer) {
        for (const auto& neighbor : node.neighbors[layer]) {
            ...
            nlist.erase(
                std::remove_if(nlist.begin(), nlist.end(),
                               [node_id](const HnswNeighbor& n) {
    ...
    if (wal_ != nullptr) {
        ...
        (void)log_wal(WalRecordType::DELETE, loc.page_id, loc.slot_id,
                      node_bytes);
    }
...
Result<void> HnswIndex::compact() {
    ...
    // Collect tombstoned node IDs.
    std::vector<uint32_t> tombstoned;
    for (const auto& [nid, nloc] : node_map_) {
        auto node = read_node(nloc);
        if (node.has_value() && node.value().is_tombstone()) {
            tombstoned.push_back(nid);
        }
    }

    // Delete tombstoned nodes' page tuples and remove from node_map_.
    for (uint32_t nid : tombstoned) {
    ...
    GIODB_LOG_INFO("HNSW compaction: freed {} tombstoned nodes", tombstoned.size());
...
Result<HnswNodeLocation> HnswIndex::allocate_node(const HnswNode& node) {
    ...
    if (!slot_result.has_value()) {
        ...
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to insert node into HNSW page");
    }
...
Result<void> HnswIndex::update_node(const HnswNodeLocation& loc,
                                     const HnswNode& node) {
    auto data = serialize_hnsw_node(node);
    auto fetch_result = buffer_pool_.fetch_page(loc.page_id);
    if (!fetch_result.has_value()) {
    ...
    if (!update_result.has_value()) {
        ...
        if (!new_loc_result.has_value()) {
            return make_error(StatusCode::INTERNAL_ERROR,
                              "Failed to re-allocate grown HNSW node");
        }
...
Result<HnswNodeLocation> HnswIndex::allocate_vector(std::span<const float> vec) {
    ...
    if (!fetch_result.has_value()) {
        return make_error(StatusCode::IO_ERROR,
                          "Failed to fetch HNSW vector data page");
    }
    Page* page = fetch_result.value();
    ...
    if (!slot_result.has_value()) {
        (void)buffer_pool_.unpin_page(page_id, false);
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to insert vector into HNSW page");
    }
    SlotId slot_id = slot_result.value();
...
Result<std::vector<float>> HnswIndex::read_vector(PageId page_id,
...
Result<void> HnswIndex::flush_meta() {
    ...
    if (!fetch_result.has_value()) {
        return make_error(StatusCode::IO_ERROR,
                          "Failed to fetch HNSW metadata page for flush");
    }
    ...
    if (!update_result.has_value()) {
        (void)buffer_pool_.unpin_page(meta_page_id_, false);
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to update HNSW metadata on page");
    }
...
Result<HnswNodeLocation> HnswIndex::node_location(uint32_t node_id) const {
    ...
    if (it == node_map_.end()) {
        return make_error(StatusCode::NOT_FOUND,
                          "HNSW node not found in location map");
    }
    return ok(it->second);
}
...
Result<PageId> HnswIndex::find_page_with_space(PageType type, size_t min_free) {
    ...
    if (!new_page_result.has_value()) {
        return make_error(StatusCode::INTERNAL_ERROR,
                          "Failed to allocate new HNSW page");
    }
...
uint8_t HnswIndex::random_layer() {
    ...
    auto level = static_cast<uint8_t>(
        std::floor(-std::log(dist(rng_)) * m_l));
    return level;
}

float HnswIndex::distance_l2(std::span<const float> a,
...
Result<uint32_t>
HnswIndex::greedy_search_layer(std::span<const float> query,
                                uint32_t entry_id,
                                uint8_t layer) const {
    ...
    if (!entry_node_result.has_value()) {
        return make_error(entry_node_result.error().code,
                          entry_node_result.error().message);
    }
    auto entry_vec_result = read_node_vector(entry_node_result.value());
    if (!entry_vec_result.has_value()) {
        return make_error(entry_vec_result.error().code,
                          entry_vec_result.error().message);
    }
...
HnswIndex::search_layer(std::span<const float> query,
    ...
    auto cmp_max = [](const Candidate& a, const Candidate& b) {
        return a.distance < b.distance;
    };

    std::priority_queue<Candidate, std::vector<Candidate>, decltype(cmp_min)>
        candidates(cmp_min);
    std::priority_queue<Candidate, std::vector<Candidate>, decltype(cmp_max)>
        result(cmp_max);
...
HnswIndex::select_neighbors(const std::vector<Candidate>& candidates,
                              uint16_t max_neighbors) const {
    std::vector<HnswNeighbor> selected;
    selected.reserve(std::min(static_cast<size_t>(max_neighbors),
                              candidates.size()));
...
Result<void> HnswIndex::log_wal(WalRecordType type, PageId page_id,
                                 SlotId slot_id,
