...
// Exhaustive invalid transitions from each state.

TEST_F(QA144ConnectionTest, InitToInitInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    auto r = conn.transition_to(ConnectionState::INIT);
    EXPECT_FALSE(r.has_value());
}

TEST_F(QA144ConnectionTest, InitToQueryInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    auto r = conn.transition_to(ConnectionState::QUERY);
    EXPECT_FALSE(r.has_value());
}

TEST_F(QA144ConnectionTest, InitToClosedInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    // INIT -> CLOSED is NOT valid per the state machine (must go through AUTH first).
    auto r = conn.transition_to(ConnectionState::CLOSED);
    EXPECT_FALSE(r.has_value());
}

TEST_F(QA144ConnectionTest, AuthToInitInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
...
TEST_F(QA144ConnectionTest, AuthToQueryInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    ASSERT_TRUE(conn.transition_to(ConnectionState::AUTH).has_value());
    auto r = conn.transition_to(ConnectionState::QUERY);
...
TEST_F(QA144ConnectionTest, QueryToInitInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    ASSERT_TRUE(conn.transition_to(ConnectionState::AUTH).has_value());
...
TEST_F(QA144ConnectionTest, QueryToAuthInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    ASSERT_TRUE(conn.transition_to(ConnectionState::AUTH).has_value());
    ASSERT_TRUE(conn.transition_to(ConnectionState::READY).has_value());
    ASSERT_TRUE(conn.transition_to(ConnectionState::QUERY).has_value());
    auto r = conn.transition_to(ConnectionState::AUTH);
    EXPECT_FALSE(r.has_value());
}

TEST_F(QA144ConnectionTest, ClosedToAnyInvalid) {
    Connection conn(fd_a_);
    fd_a_ = -1;
    ASSERT_TRUE(conn.transition_to(ConnectionState::AUTH).has_value());
    ASSERT_TRUE(conn.transition_to(ConnectionState::CLOSED).has_value());

    EXPECT_FALSE(conn.transition_to(ConnectionState::INIT).has_value());
    EXPECT_FALSE(conn.transition_to(ConnectionState::AUTH).has_value());
    EXPECT_FALSE(conn.transition_to(ConnectionState::READY).has_value());
    EXPECT_FALSE(conn.transition_to(ConnectionState::QUERY).has_value());
    EXPECT_FALSE(conn.transition_to(ConnectionState::CLOSED).has_value());
}

// Full lifecycle: INIT -> AUTH -> READY -> QUERY -> READY -> QUERY -> READY -> CLOSED
TEST_F(QA144ConnectionTest, FullLifecycleCycle) {
    Connection conn(fd_a_);
    fd_a_ = -1;
