...
TEST_F(QA_PlannerBugs, GDB229_DeeplyNestedMixedSubqueries) {
    ...
    // GDB-238 fix: correlated EXISTS inside OR now evaluates via manual
    // correlation at runtime (scan inner table, combine with outer tuple).
    auto qr = exec_ok(
    ...
    // alice (dept 10 → OR short-circuits) and charlie (dept 10 → OR short-circuits).
    // bob (dept 20, no orders) and diana (dept 30, no orders) filtered out.
    auto names = collect_column_strings(qr, 0);
    EXPECT_EQ(names.size(), 2u);
    EXPECT_TRUE(names.count("alice"));
    EXPECT_TRUE(names.count("charlie"));
...
TEST_F(QA_PlannerBugs, GDB231_NotInWithAllNullsInSubquery) {
    ...
    EXPECT_EQ(qr.rows.size(), 0u) << "NOT IN with all-NULL subquery should return 0 rows";
}
...
TEST_F(QA_PlannerBugs, CrossBug_ExistsAndInWithCTE_GDB229_GDB232) {
    ...
    auto qr = exec_ok("WITH active AS (SELECT departments.id FROM departments "
                      "WHERE departments.dept_name = 'engineering') "
                      "SELECT users.name FROM users "
                      "WHERE EXISTS (SELECT 1 FROM orders WHERE orders.user_id = users.id) "
...
TEST_F(QA_PlannerBugs, CrossBug_ScalarSubqueryAndInWithNulls_GDB230_GDB231) {
    ...
    auto qr = exec_ok("SELECT users.name FROM users "
                      "WHERE users.id = (SELECT orders.user_id FROM orders WHERE orders.id = 100) "
                      "AND users.id NOT IN (SELECT nullable_ids.val FROM nullable_ids WHERE "
                      "nullable_ids.val IS NOT NULL)");
...
TEST_F(QA_PlannerBugs, CrossBug_AllFourFixes_GDB229_230_231_232) {
    ...
    auto qr = exec_ok("WITH target_depts AS (SELECT departments.id FROM departments "
                      "WHERE departments.dept_name = 'engineering') "
                      "SELECT users.name FROM users "
                      "WHERE EXISTS (SELECT 1 FROM orders WHERE orders.user_id = users.id) "
