...
TEST(PgProtocol, StartupHandshake) {
    ...
    auto startup = pg_wire_test::build_startup_message({{"user", "test"}, {"database", "testdb"}});
    ...
    auto response = pg_wire_test::read_from_fd(client_fd);
...
TEST(PgProtocol, SslRequestDenied) {
    ...
    pg_wire_test::write_to_fd(client_fd, ssl_req);
...
TEST(PgProtocol, SimpleQuerySelect) {
    ...
    (void)pg_wire_test::read_from_fd(client_fd); // Discard startup response.

    ASSERT_EQ(handler.state(), ProtocolState::READY);

    // Send a query.
    auto query = pg_wire_test::build_query_message("SELECT id, name FROM users");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    auto process_result = handler.process(conn);
    ASSERT_TRUE(process_result.has_value());
...
TEST(PgProtocol, SimpleQueryError) {
    ...
    // Send a bad query.
    auto query = pg_wire_test::build_query_message("SELEKT * FROM oops");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    auto process_result = handler.process(conn);
    ASSERT_TRUE(process_result.has_value());
...
TEST(PgProtocol, EmptyQuery) {
    ...
    // Send empty query.
    auto query = pg_wire_test::build_query_message("");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    auto process_result = handler.process(conn);
    ASSERT_TRUE(process_result.has_value());
...
TEST(PgProtocol, TerminateMessage) {
    ...
    pg_wire_test::write_to_fd(client_fd, terminate);
    (void)conn.read_from_socket();
    auto process_result = handler.process(conn);
    ASSERT_TRUE(process_result.has_value());
...
TEST(PgProtocol, DmlQueryInsert) {
    ...
    // Send insert query.
    auto query = pg_wire_test::build_query_message("INSERT INTO t VALUES (1, 2, 3)");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    auto process_result = handler.process(conn);
    ASSERT_TRUE(process_result.has_value());
...
TEST(PgProtocol, DdlQuery) {
    ...
    auto query = pg_wire_test::build_query_message("CREATE TABLE t (id INT)");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    (void)handler.process(conn);
    (void)conn.write_to_socket();
...
TEST(PgProtocol, PartialMessageReassembly) {
    ...
    // Send first part.
    pg_wire_test::write_to_fd(client_fd, part1);
    (void)conn.read_from_socket();
    ...
    // Send second part.
    pg_wire_test::write_to_fd(client_fd, part2);
    (void)conn.read_from_socket();
...
TEST(PgProtocol, DataRowNullValues) {
    ...
    // Query.
    auto query = pg_wire_test::build_query_message("SELECT val FROM t");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    (void)handler.process(conn);
    (void)conn.write_to_socket();
...
TEST(PgProtocol, ParseBindExecuteSync) {
    ...
    auto parse = pg_wire_test::build_parse_message("", "SELECT id FROM t WHERE id = $1", {23});
    auto bind = pg_wire_test::build_bind_message("", "", {std::string("42")});
...
TEST(PgProtocol, NamedPreparedStatementPersists) {
    ...
    int call_count = 0;
    handler.set_query_executor(
        [&call_count](const std::string& /*sql*/,
    ...
    auto parse = pg_wire_test::build_parse_message("mystmt", "SELECT 1");
    auto sync1 = pg_wire_test::build_sync_message();
    std::vector<uint8_t> batch1;
    batch1.insert(batch1.end(), parse.begin(), parse.end());
    batch1.insert(batch1.end(), sync1.begin(), sync1.end());

    pg_wire_test::write_to_fd(client_fd, batch1);
    (void)conn.read_from_socket();
    (void)handler.process(conn);
    (void)conn.write_to_socket();
    (void)pg_wire_test::read_from_fd(client_fd);
    ...
    for (int i = 0; i < 2; ++i) {
        auto bind = pg_wire_test::build_bind_message("", "mystmt");
        auto execute = pg_wire_test::build_execute_message("");
        auto sync = pg_wire_test::build_sync_message();
        std::vector<uint8_t> batch;
        batch.insert(batch.end(), bind.begin(), bind.end());
...
TEST(PgProtocol, CloseStatement) {
    ...
    auto parse = pg_wire_test::build_parse_message("todelete", "SELECT 1");
    auto sync1 = pg_wire_test::build_sync_message();
    std::vector<uint8_t> batch1;
    batch1.insert(batch1.end(), parse.begin(), parse.end());
    batch1.insert(batch1.end(), sync1.begin(), sync1.end());
    ...
    auto sync2 = pg_wire_test::build_sync_message();
    ...
    pg_wire_test::write_to_fd(client_fd, batch2);
    (void)conn.read_from_socket();
    (void)handler.process(conn);
    (void)conn.write_to_socket();
    auto response = pg_wire_test::read_from_fd(client_fd);
...
TEST(PgProtocol, DescribeStatement) {
    ...
    auto parse = pg_wire_test::build_parse_message("desc_test", "SELECT $1", {23, 25});
...
TEST(PgProtocol, DescribeStatementRowDescription) {
    ...
    handler.set_query_describer([](const std::string& sql, const std::string& /*database*/)
                                    -> Result<std::vector<ColumnDescription>> {
    ...
    auto parse = pg_wire_test::build_parse_message("sel_stmt", "SELECT id, name FROM t", {});
...
TEST(PgProtocol, DescribeStatementNoDataForDML) {
    ...
    auto parse = pg_wire_test::build_parse_message("ins_stmt", "INSERT INTO t VALUES (1)", {});
...
TEST(PgProtocol, DescribePortalRowDescription) {
    ...
    auto parse = pg_wire_test::build_parse_message("", "SELECT id, name FROM t");
    auto bind = pg_wire_test::build_bind_message("myportal", "");
...
TEST(PgProtocol, DescribePortalNoDataForDML) {
    ...
    auto parse = pg_wire_test::build_parse_message("", "INSERT INTO t VALUES (1)");
    auto bind = pg_wire_test::build_bind_message("myportal", "");
    auto describe = build_describe_message('P', "myportal");
    auto sync = pg_wire_test::build_sync_message();
...
TEST(PgProtocol, DescribeStatementDescriberError) {
    ...
    auto parse = pg_wire_test::build_parse_message("bad_stmt", "SELECT * FROM nonexistent");
...
TEST(PgProtocol, DescribePortalDescriberError) {
    ...
    auto parse = pg_wire_test::build_parse_message("", "SELECT * FROM nonexistent");
    auto bind = pg_wire_test::build_bind_message("bad_portal", "");
...
TEST(PgProtocol, ErrorInBatchSkipsUntilSync) {
    ...
    auto bind = pg_wire_test::build_bind_message("", "nonexistent");
    auto execute = pg_wire_test::build_execute_message("");
    auto sync = pg_wire_test::build_sync_message();
...
TEST(PgProtocol, MultiStatementErrorStopsExecution) {
    ...
    auto query = pg_wire_test::build_query_message("SELECT 1; BAD SQL; SELECT 3");
    pg_wire_test::write_to_fd(client_fd, query);
    (void)conn.read_from_socket();
    auto result = handler.process(conn);
    ASSERT_TRUE(result.has_value());
