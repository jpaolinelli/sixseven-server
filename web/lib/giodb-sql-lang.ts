/**
 * GioDB SQL language support for CodeMirror 6.
 *
 * Extends the standard SQL dialect with GioDB-specific keywords
...
// Custom dialect that adds GioDB keywords to SQL
const giodbDialect = SQLDialect.define({
  keywords:
    SQL_KEYWORDS.join(" ").toLowerCase() +
    " " +
    GIODB_KEYWORDS.join(" ").toLowerCase(),
  types:
...
/**
 * Build a CodeMirror autocomplete source from GioDB schema data.
 */
export function giodbCompletionSource(schemaData: SchemaCompletionData) {
  return (context: CompletionContext): CompletionResult | null => {
    ...
    // SQL + GioDB keywords
    const allKeywords = [...SQL_KEYWORDS, ...GIODB_KEYWORDS];
    for (const kw of allKeywords) {
      options.push({
        label: kw,
        type: "keyword",
        boost: GIODB_KEYWORDS.includes(kw) ? 1 : 0,
      });
    }
...
/**
 * Create the GioDB SQL language extension for CodeMirror.
 */
export function giodbSQL(config?: Partial<SQLConfig>) {
  return sql({
    dialect: giodbDialect,
